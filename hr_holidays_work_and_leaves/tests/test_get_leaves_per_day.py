...
class TestGetLeavesPerDay(WorkAndLeavesBase):
    def test_two_leaves_returned_in_period(self):
        """Full-day leave gives midnight-to-midnight entry; partial leave keeps stored times."""
        ...
        # Partial leave: Wednesday 2026-05-13 09:00–13:00 UTC.
        self._make_validated_leave(
            self.employee,
        ...
        first, second = leaves
        # Full-day leave: spans the whole calendar day midnight-to-midnight so
        # the merge algorithm clips it to whatever the current schedule says.
        self.assertEqual(first.type, "leave")
        self.assertEqual(first.datetime_from.date(), date(2026, 5, 11))
        self.assertEqual(first.datetime_from.timetz().replace(tzinfo=None), time(0, 0))
        self.assertEqual(first.datetime_to.date(), date(2026, 5, 12))
        self.assertEqual(first.datetime_to.timetz().replace(tzinfo=None), time(0, 0))
        self.assertAlmostEqual(first.duration, 24.0)
        # Partial leave: stored start/end times are used unchanged.
        ...
        self.assertAlmostEqual(second.duration, 4.0)
