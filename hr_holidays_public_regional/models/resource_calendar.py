...
class ResourceCalendar(models.Model):
    ...
    def _attendance_intervals_batch_exclude_public_holidays_regional(
        self, start_dt, end_dt, intervals, resources, tz
    ):
        employee_id = self.env.context.get("employee_id", False)
        employee = self.env["hr.employee"].browse(employee_id)
        list_by_dates = (
            self.env["hr.holidays.public.regional"]
            .get_regional_holidays_list(
    ...
    def _attendance_intervals_batch(
        ...
        if self.env.context.get("exclude_public_regional_holidays") and resources:
            return self._attendance_intervals_batch_exclude_public_holidays_regional(
