...
class TestNotifyEmployeeManager(TransactionCase):
    def setUp(self):
        ...
        self.emp_manager = self.emp_model.create(
            {"name": "Test Manager", "user_id": self.manager.id}
        )
    ...
    def test_add_follower_1(self):
        ...
        follower_set = (
            manager in leave.message_follower_ids.mapped("partner_id")
            if manager
            else False
        )
    ...
    def test_add_follower_2(self):
        ...
        self.assertTrue(
            follower_set, "Employee's manager hasn't been added " "as follower."
        )
