...
class HrLeave(models.Model):
    ...
    request_hour_from = fields.Float(compute="_compute_hour_from", store=True)

    request_time_hour_to = fields.Float("Float hour to")

    request_hour_to = fields.Float(compute="_compute_hour_to", store=True)

    @api.depends("request_time_hour_from")
    def _compute_hour_from(self):
        for leave in self:
            leave.request_hour_from = leave.request_time_hour_from

    @api.depends("request_time_hour_to")
    def _compute_hour_to(self):
        for leave in self:
            leave.request_hour_to = leave.request_time_hour_to
